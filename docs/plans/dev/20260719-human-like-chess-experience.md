...
- [x] Показывать на главной странице перед разделом конфиденциальности агрегаты реальных пользователей по умолчанию и динамику за месяц/год/всё время без раскрытия идентификаторов. **Вкладки test/all не реализованы намеренно:** публичная страница показывает только `real`, а служебный трафик виден операторам в БД. Публиковать счётчики автоматических проверок на витрине смысла нет.
...
- [x] Озвучивать каждый полуход ответа собственным сигналом: ход игрока звучит в начале, ход движка — на словах «Мой ход». Для каждой половины выбирается самый значимый сигнал: победа, мат, шах, ход; «начало» звучит только тогда, когда в ответе не прозвучал ни один ход
...
## Task 22 — Голосовая справка по правилам шахмат

**Status:** completed 2026-07-30

**Evidence:** продуктовый анализ выявил `10` обращений от `10` пользователей в общем кластере правил, из них `8` не распознаны; на текущем релизе сохраняются `9` обращений от `7` пользователей.

**Implementation:**

- `src/yura_chess/presentation/help_speech.py`
  - добавить `HelpTopic.RULES` и отдельный постраничный раздел;
  - покрыть цель игры, шах/мат, ходы фигур, рокировку, превращение, взятие на проходе и пат;
  - распознавать морфологические варианты правил и обучения только при явном слове с основой `шахмат`.
- `src/yura_chess/application/command_router.py`
  - направлять общие вопросы о правилах в `CommandKind.HELP` до распознавания ходов;
  - не перехватывать команды начала игры, platform/device intents и нешахматные правила.
- `tests/presentation/test_help_speech.py`
  - проверить содержание всех страниц и лимиты Alice/TTS.
- `tests/voice/test_move_resolver.py`
  - проверить естественные формулировки и отрицательные маршрутизационные примеры.
- `tests/application/test_conversation.py`
  - проверить меню, выбор раздела, пагинацию, replay safety и отсутствие изменения партии.

**Acceptance:**

- [x] Общие запросы о правилах открывают раздел «правила».
- [x] Раздел доступен из меню и по фразе «справка про правила».
- [x] Все заявленные основы шахмат покрыты короткими голосовыми страницами.
- [x] Просмотр справки не меняет игру и повтор запроса идемпотентен.
- [x] Соседние intents не перехватываются.
- [x] Ответы проходят лимиты Alice text/TTS.
