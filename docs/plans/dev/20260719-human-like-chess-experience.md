...
- [ ] Добавить типизированные разделы справки, команды общего меню, отдельных тем, полного каталога, навигации и выхода
- [ ] Формировать короткие mode-aware ответы до игры, в обычной партии, в тренировке, после партии и в задаче
- [ ] При активной справке обрабатывать «дальше», «назад» и «сначала» раньше позиционной пагинации
- [ ] Гарантировать отсутствие изменений партии, ревизии, pending turn и попытки задачи
- [ ] Добавить router и conversation тесты всех разделов, навигации, неизвестной темы и выхода
- [ ] Mark completed
...
- [ ] Реализовать ответы о цвете, номере хода, сыгранных полных ходах и снятых фигурах из канонической истории
- [ ] Реализовать права и текущую доступность рокировки с конкретной причиной
- [ ] Называть шах и атакующие фигуры, а также изменения последнего хода
- [ ] Гарантировать одинаковые ответы после reload и отсутствие мутаций
- [ ] Добавить естественные фразы и граничные позиции в focused tests
- [ ] Mark completed
...
- [ ] Добавить команды краткости, подробности, добавления/удаления пунктуационных пауз, стиля нотации и фиксированной экранной ориентации
- [ ] Применять предпочтения ко всем подходящим ответам без изменения шахматного смысла
- [ ] Не обещать управление физической скоростью Alice TTS: slow добавляет паузы, fast убирает только добавленные навыком паузы
- [ ] Реализовать реванш тем же цветом, смену цвета и усиление следующей партии на 2 уровня с cap 20 и подтверждением результата
- [ ] Не допускать распознавания команды настройки как шахматного хода
- [ ] Проверить новую Alice-сессию, обе ориентации и наследование уровня
- [ ] Mark completed
...
- [ ] Распознавать наиболее длинный UCI-префикс и возвращать «дебют не определён» без влияния на игру
- [ ] Реализовать документированную эвристику дебюта, миттельшпиля и эндшпиля
- [ ] Покрыть неизвестные последовательности, ранний размен ферзей и граничные позиции стадии
- [ ] Mark completed
...
- [ ] Реализовать включение/выключение тренера и предложение включить его при запросе совета в обычной партии
- [ ] Реализовать словесную/числовую оценку, цель последнего хода, угрозу и до трёх кандидатов
- [ ] Анализировать предложенный legal move на копии board; illegal/ambiguous обрабатывать существующим resolver/explainer
- [ ] Реализовать четыре ступени подсказки и идемпотентное продвижение ровно на одну ступень
- [ ] После принятого пользовательского хода в training анализировать вне DB-транзакции и идемпотентно сохранять checkpoint до ответа движка
- [ ] Реализовать «где я ошибся» как последний checkpoint с потерей не менее 100 сантипешек, а также предупреждение, «оставить мой ход» и «вернуть ход» без скрытой замены хода
- [ ] Покрыть busy, timeout, replay и отсутствие мутаций UCI/revision/pending turn
- [ ] Mark completed
...
- [ ] Реализовать результат, перелом, главную ошибку, число ошибок и лучший практический ход по единым порогам 50/100/200 сантипешек
- [ ] Для каждого пользовательского хода переиспользовать существующий checkpoint либо выполнить read-only анализ вне DB-транзакции и сохранить результат; при timeout вернуть честный частичный разбор с возможностью продолжить
- [ ] Продолжать длинный разбор через server-side review state и восстанавливать выбранные game/ply после новой сессии
- [ ] Реализовать подтверждённую тренировочную ветку из позиции перелома без изменения завершённой партии
- [ ] Экспортировать standards-compliant PGN и читать ходы постранично голосом
- [ ] Проверить PGN round-trip, interruption/resume и неизменность завершённой игры
- [ ] Mark completed
...
- [ ] Добавить в справку все фактические вопросы, preferences, training, review/PGN и puzzle команды, реализованные в Tasks 3–16
- [ ] Для недоступной в текущем состоянии команды объяснять требуемый режим или следующий шаг, не рекламируя несуществующие возможности
- [ ] Проверить, что каждая публичная категория команд представлена ровно в одном разделе и полный каталог остаётся постраничным
- [ ] Повторно доказать отсутствие мутаций game/revision/pending turn/review/puzzle attempt при любой навигации справки
- [ ] Mark completed
