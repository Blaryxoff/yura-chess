...
_PATTERNS: tuple[tuple[GameFact, re.Pattern[str]], ...] = (
    ...
    (
        ...
        re.compile(
            r"кто (дает|дал|объявил|поставил|ставит) шах|чем( мне| нам)? шах|"
            r"как\w+ фигур\w* атакует \w*корол|кто атакует \w*корол"
        ),
    ),
    (
        GameFact.CAPTURED,
        re.compile(
            ...
            r"сколько фигур (съеден|снят|побит|сбит|потерян|взят)|"
            r"какие фигуры съел|съеденные фигуры|снятые фигуры"
        ),
    ),
    (
