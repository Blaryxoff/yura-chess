...
def _users_hint(form: str) -> str:
    return (
        ...
        "Мы не сохраняем исходный идентификатор Алисы. Для подсчёта повторных визитов используется необратимый код: "
        "восстановить по нему идентификатор нельзя. Запросы и сессии тоже считаются без открытых идентификаторов.</span>"
    )
