...
def describe_move(board_before: chess.Board, move: chess.Move) -> Speech:
    """Describe `move` in the position it is played in."""
    board_after = board_before.copy(stack=False)
    board_after.push(move)
    return Speech.of(_body(board_before, move) + _suffix(board_after))


def describe_played_move(board_after: chess.Board, move: chess.Move) -> Speech:
    ...
    return Speech.of(f"{name} {source} {destination}." + _suffix(board_after))


def _body(board: chess.Board, move: chess.Move) -> str:
    ...
    if board.is_en_passant(move):
        return f"{name} {source} берет пешку на {destination} на проходе."
    captured = board.piece_at(move.to_square)
    if captured is not None:
        body = f"{name} {source} берет {PIECE_NAMES_ACCUSATIVE[captured.piece_type]} на {destination}"
    else:
        body = f"{name} {source} {destination}"
