...
LANDING_FAQ: tuple[tuple[str, str], ...] = (
    ...
    (
        ...
        "против движка Stockfish с уровнями сложности от 0 до 20.",
    ),
    (
    ...
    (
        ...
        "Да. Тренер оценивает позицию, предлагает варианты и подсказывает — сначала намёком, "
        "затем конкретным ходом. После партии навык находит перелом, главную ошибку и показывает запись в PGN.",
    ),
    (
        "Как узнать все команды?",
        "Скажите «что ты умеешь?» для короткого меню или «все команды» для полного списка. "
...
_LANDING_FAQ_COMMANDS = (
    "Алиса, запусти навык Шахматы с Юрой",
    "новая игра белыми уровень пять",
...
def _highlight_landing_faq_commands(answer: str) -> str:
    for command in _LANDING_FAQ_COMMANDS:
        phrase = f"«{command}»"
        answer = answer.replace(phrase, f"<code>{phrase}</code>")
    return answer
...
LANDING_FAQ_HTML = "\n".join(
    f"""        <div>
          <dt>{question}</dt>
          <dd>{_highlight_landing_faq_commands(answer)}</dd>
        </div>"""
    for question, answer in LANDING_FAQ
)
...
{_launch_cta("Скажите Алисе")}
    ...
    <section>
      ...
      <div class="grid">
        <div class="feature">
          ...
          <code>«Пешка е два е четыре»</code>, <code>«конь эф три»</code>,
          <code>«короткая рокировка»</code> — навык понимает разные формы шахматной речи.
        </div>
        <div class="feature">
          <strong>Играйте на своём уровне</strong>
          Выбирайте цвет и силу Stockfish от спокойной партии до серьёзной тренировки. Хотите учиться,
          а не только играть — включите <a href="{COACH_PATH}">тренера с оценкой позиции и подсказками</a>.
        </div>
        <div class="feature">
        ...
        <div class="feature">
          ...
          Хотите попробовать играть, не слушая всю позицию, — начните с
          <a href="{BLINDFOLD_PATH}">упражнения для игры вслепую</a>.
        </div>
        <div class="feature">
    ...
    <section>
      ...
      <p class="section-intro">
        Это только примеры. Скажите <code>«что ты умеешь?»</code> или <code>«все команды»</code>,
        чтобы услышать полный список,
...
{LANDING_FAQ_HTML}
    ...
    <section>
      ...
      <p>
        ...
        запросов. Мы не храним исходный идентификатор Алисы и не можем восстановить его из сохранённых данных.
        Обезличенные счётчики использования сохраняются без срока для статистики, а текст распознанных команд —
        до 30 дней для улучшения качества распознавания.
      </p>
      <p>Навык не использует покупки, рекламу или привязку сторонних аккаунтов. Исходный код проекта доступен на <a href="https://github.com/Blaryxoff/yura-chess">GitHub</a>.</p>
    </section>
"""
    ...
    <section class="article">
      ...
      <ol class="steps">
        <li>
          ...
          Скажите <code>«Алиса, запусти навык Шахматы с Юрой»</code>. Навык бесплатный,
          устанавливать ничего не нужно.
        </li>
        <li>
          <strong>Начните партию</strong>
          Скажите <code>«новая игра белыми уровень пять»</code> или <code>«новая игра чёрными»</code>.
          Уровень Stockfish — от нуля до двадцати: ноль подходит начинающим, двадцать — максимальный уровень.
        </li>
        <li>
          <strong>Называйте ходы обычными словами</strong>
          Подойдут <code>«пешка е два е четыре»</code>, <code>«конь эф три»</code>
          или просто <code>«е два е четыре»</code>. Когда позиция позволяет, можно сказать
          <code>«слон берёт на аш семь»</code> или <code>«короткая рокировка»</code>.
          Если фраза допускает несколько ходов, навык переспросит
          и не станет ходить наугад.
        </li>
        <li>
          <strong>Спрашивайте о доске</strong>
          <code>«Какая позиция»</code> читает доску по горизонталям, <code>«что на эф три»</code> называет
          фигуру на клетке. Можно спросить <code>«где белые слоны»</code>, <code>«чей ход»</code>,
          <code>«есть ли шах»</code> или <code>«какой был последний ход»</code> — всё доступно на слух.
        </li>
        <li>
          <strong>Возвращайтесь к партии</strong>
          Незаконченная игра сохраняется. При следующем запуске скажите
          <code>«продолжить последнюю партию»</code>:
          Алиса напомнит последние ходы и продолжит с той же позиции.
        </li>
      </ol>

      <h3>Короткий пример: запуск, первый ход, возврат к партии</h3>
      <p>
        Вот несколько коротких фраз, которые пригодятся во время партии.
      </p>
      <ul>
        <li>
      ...
      <p>
        Хотите играть с подсказками? Посмотрите, <a href="{COACH_PATH}">как работает режим тренера</a>.
        Для тренировки без доски попробуйте <a href="{BLINDFOLD_PATH}">упражнение на десять ходов</a>.
      </p>
      ...
      <p>
        ...
        Если Алиса неточно распознала фразу, скажите <code>«что ты услышала»</code> или
        <code>«повтори координаты по буквам»</code>.
      </p>

      <h3>Как выбрать уровень Stockfish</h3>
      <p>
        Уровень Stockfish можно выбрать от 0 до 20. Начните с уровня 5, а затем подберите удобную сложность.
        Спросите <code>«какой уровень»</code>, чтобы узнать текущий, и скажите
        <code>«сыграем сложнее»</code>, чтобы поднять сложность в следующей партии.
      </p>

      <h3>Тренер, разбор и задачи</h3>
      <p>
        Скажите <code>«включи режим тренера»</code>, чтобы получать оценку позиции, несколько подходящих ходов
        и подсказки от общего намёка к конкретному ходу. После партии команда
        <code>«разбери партию»</code> найдёт перелом и главную ошибку, а <code>«дай задачу»</code> откроет
      ...
      <p>
        В карточке навыка Яндекс указывает Станции и другие умные колонки, телефоны, Навигатор, Карты,
        Яндекс Авто и компьютеры с Windows. <a href="{YANDEX_DIALOG_URL}#surfaces">Актуальный список устройств</a>
        хранится там же. Экран для игры не нужен: доска на нём только дополняет голосовые ответы.
      </p>
    </section>
""",
)

_COMMANDS_TITLE = "Голосовые команды для шахмат с Алисой"
_COMMANDS_CRUMB_SCHEMA = _breadcrumb_schema(_COMMANDS_TITLE, COMMANDS_PATH)

COMMANDS_PAGE_HTML = _document(
    title="Голосовые команды для шахмат с Алисой — полный список",
    description=(
...
{_launch_cta()}
    ...
    <section class="article">
      ...
      <p>Во время партии можно проверить, что услышала Алиса, или попросить повторить ответ.</p>
      <ul>
      ...
      <p>
        Скажите <code>«что ты умеешь»</code> для короткого меню, <code>«все команды»</code> для полного списка
        или назовите раздел. Пока справка открыта, можно сказать <code>«ходы»</code>, <code>«позиция»</code>,
        <code>«факты»</code>, <code>«партия»</code>, <code>«настройки»</code>, <code>«тренер»</code>,
        <code>«разбор»</code> или <code>«задачи»</code>. Перемещаться по открытой справке помогают
...
{_launch_cta()}
    ...
    <section class="article">
      ...
      <p>
        Команда <code>«какая позиция»</code> читает доску по две горизонтали за раз, чтобы их было легче
        запомнить на слух; <code>«дальше»</code> продолжает чтение. Отдельные вопросы уточняют детали:
        <code>«что на е четыре»</code>, <code>«где белые слоны»</code>, <code>«чей ход»</code>,
        <code>«есть ли шах»</code>, <code>«какие фигуры съедены»</code>.
      </p>

      <h3>Историю партии не нужно держать в голове</h3>
      <p>
        <code>«Какой был последний ход»</code>, <code>«что делали чёрные четыре хода назад»</code>,
        <code>«сколько ходов мы сыграли»</code>, <code>«что изменил последний ход»</code> — навык помнит
        партию целиком и отвечает на слух. Незаконченная игра сохраняется между сессиями:
        <code>«продолжить последнюю партию»</code> вернёт позицию и напомнит последние ходы.
      </p>

      <h3>Распознавание можно проверить</h3>
      <p>
        Ошибка распознавания не приводит к случайному ходу. Если фразу можно понять по-разному,
        навык переспрашивает и не меняет позицию. Во время партии команда <code>«что ты услышала»</code>
        повторяет распознанную фразу, команды <code>«повтори ответ»</code> и <code>«повтори медленно»</code>
        повторяют предыдущий ответ, а <code>«повтори координаты по буквам»</code> проговаривает поле по буквам —
        на случай, когда Алиса путает «б» и «п».
      </p>

      <h3>Темп и подробность настраиваются</h3>
      <p>
        <code>«Говори кратко»</code> и <code>«говори подробно»</code> меняют объём ответов,
        <code>«говори медленнее»</code> добавляет паузы между фразами, а <code>«короткая нотация»</code>
        и <code>«полная нотация»</code> меняют стиль произнесения ходов. Настройки сохраняются между партиями,
        поэтому подбирать их заново не нужно.
      </p>

      <h2>Тренер и задачи тоже без экрана</h2>
      <p>
        <a href="{COACH_PATH}">Режим тренера</a> озвучивает оценку позиции, хорошие ходы, угрозы
        и подсказки от общего намёка к конкретному ходу. Разбор сыгранной партии называет перелом
        и главную ошибку. В <a href="{PUZZLES_PATH}">голосовых задачах</a> позиция и подсказки звучат
        вслух, а готовое решение можно попросить отдельно. Ничего из этого не требует зрительного контакта с доской.
      </p>
      <p>
      ...
      <h2>Как начать</h2>
      <p>
        Скажите <code>«Алиса, запусти навык Шахматы с Юрой»</code>, затем
        <code>«новая игра белыми уровень пять»</code>.
...
COACH_PAGE_HTML = _document(
    ...
    description=(
        ...
        "угрозы, подсказки от намёка к ходу и разбор сыгранной партии — всё голосом."
    ),
    path=COACH_PATH,
...
{_launch_cta()}
    ...
    <section class="article">
      ...
      <p>
        ...
        Команда <code>«выключи тренера»</code> возвращает обычную игру без оценок и подсказок.
      </p>

      <h2>Пример: один голосовой цикл целиком</h2>
      <p>
        Если попросить оценку при выключенном тренере, навык сначала предложит включить его.
        После ответа <code>«да»</code> тренер сразу оценит позицию. Например:
      </p>
      <ul>
        <li>
          <strong>Вы:</strong> <code>«оцени позицию»</code> — тренер пока выключен.
          <strong>Навык:</strong> Сейчас играем без подсказок. Включить режим тренера и ответить
          на ваш вопрос? Его можно снова выключить в любой момент. Скажите <code>«да»</code> или <code>«нет»</code>.
        </li>
        <li>
          <strong>Вы:</strong> <code>«да»</code>.
          <strong>Навык:</strong> Включаю режим тренера. У вас небольшой перевес.
          Скажите <code>«назови оценку числом»</code>, если нужна цифра.
        </li>
      </ul>
      ...
      <p>
        ...
        и за счёт чего он возник. <code>«Назови оценку числом»</code> даёт привычную шахматистам числовую
      ...
      <p>
        ...
        позиция на доске не меняется. Так можно проверить идею до того, как сделать этот ход в партии.
      </p>

      <h2>Подсказки: от намёка к ходу</h2>
      <p>
        <code>«Подскажи»</code> не выдаёт готовый ход сразу. Сначала тренер намекает на идею, затем
        называет участок доски и лишь после этого предлагает ход. Можно остановиться, как только подсказки достаточно.
      ...
      <p>
        Разобрать можно и партию, сыгранную без тренера. После её завершения навык оценит ваши ходы
        и назовёт ключевые ошибки.
      </p>
...
{_launch_cta()}
    ...
    <section class="article">
      ...
      <ul>
        ...
        <li><strong>Сквозной удар</strong> — нападение на две фигуры по одной линии: более ценная отходит,
            а стоящая за ней остаётся под ударом</li>
      ...
      <p>
        ...
        <code>«Подскажи»</code> сначала даёт намёк, а при повторном вопросе уточняет его. Команда
        <code>«покажи решение»</code> продиктует оставшиеся ходы и завершит задачу, а
        <code>«следующая задача»</code> откроет новую.
      </p>
      ...
      <p>
        Когда доска перед глазами, варианты можно проверять взглядом. На слух так не выйдет —
...
{_launch_cta()}
    ...
    <section class="article">
      ...
      <p>
        ...
        отверните экран или возьмите колонку. Соперник — Stockfish с уровнями сложности от 0 до 20,
        поэтому уровень соперника можно повышать постепенно.
      </p>
      ...
      <ol class="steps">
        ...
        <li>
          ...
          Цель тренировки — дойти как можно дальше без команды <code>«какая позиция»</code>.
        </li>
      </ol>
      ...
      <h2>Готовое упражнение: десять ходов без полного чтения доски</h2>
      <p>
        Попробуйте сыграть десять своих ходов без команды <code>«какая позиция»</code>. Точечные вопросы
        о клетке, фигуре или последнем ходе разрешены.
      </p>
      <ol class="steps">
        <li>
          <strong>Цель</strong>
          Скажите <code>«новая игра белыми уровень три»</code> и сыграйте свои первые десять ходов,
          ни разу не спросив <code>«какая позиция»</code>.
        </li>
        <li>
          <strong>Что можно</strong>
          Можно спрашивать <code>«что на е четыре»</code>, <code>«где белые слоны»</code>
          и <code>«какой был последний ход»</code>.
          Если понадобилось полное чтение доски, просто начните следующую попытку сначала.
        </li>
        <li>
          <strong>Разбор результата</strong>
          После попытки спросите <code>«сколько я сделал ходов»</code>. Доиграйте партию до конца, затем попросите
          тренера <a href="{COACH_PATH}">разобрать партию</a>. Навык назовёт ключевые ошибки и перелом,
          если он был.
        </li>
      </ol>
      ...
      <ul>
        <li>Считайте, сколько ходов удаётся продержаться без команды <code>«какая позиция»</code></li>
        ...
        <li>Между партиями решайте <a href="{PUZZLES_PATH}">задачи на слух</a> — они тоже помогают
            удерживать позицию в голове</li>
      </ul>
