    ...
    <section>
      ...
      <div class="grid">
        ...
        <div class="feature">
          ...
          Готовы обходиться совсем без этих вопросов — попробуйте
...
HOW_TO_PLAY_PAGE_HTML = _document(
    ...
    <section class="article">
      ...
      <p>
        Разобрать реальный вопрос и ответ тренера можно на странице
        <a href="{COACH_PATH}">«Шахматный тренер голосом»</a>, а потренировать память без вопросов
        о позиции — на странице <a href="{BLINDFOLD_PATH}">«Шахматы вслепую»</a>.
      </p>
      ...
      <p>
        Навык работает на любом устройстве с голосовой Алисой: умных колонках, Яндекс Станции
        и в мобильном приложении Яндекс. Экран не обязателен ни для одного действия — доска
        на экране только дополняет голосовые ответы, поэтому колонка без экрана подходит так же,
        как устройство с дисплеем.
      </p>
    </section>
""",
)
...
COACH_PAGE_HTML = _document(
    ...
    <section class="article">
      ...
      <p>
        Один и тот же вопрос до и после включения тренера отвечается по-разному — вот реальная
        последовательность вопрос — ответ — следующий шаг:
      </p>
      <ul>
        <li>
          <strong>Вы:</strong> <code>«оцени позицию»</code> — партия ещё честная, тренер выключен.
          <strong>Навык:</strong> Сейчас идёт обычная партия без подсказок. Скажите «включи режим
          тренера», и я буду оценивать позицию и подсказывать. Оценки не будет, пока тренер
          не включён явно.
        </li>
        <li>
          <strong>Вы:</strong> <code>«включи режим тренера»</code>.
          <strong>Навык:</strong> Включаю режим тренера. Я буду оценивать позицию, отвечать
          на вопросы, давать подсказки и предупреждать об ошибках. Это уже не честная партия.
        </li>
        <li>
          <strong>Вы:</strong> тот же вопрос, <code>«оцени позицию»</code>.
          <strong>Навык:</strong> например, у вас небольшой перевес. Скажите «назови оценку
          числом», если нужна цифра. Следующий шаг — сделать ход или уточнить оценку числом.
        </li>
      </ul>
      ...
      <p>
        ...
        о позиции получает только предложение включить тренера, при включённом тренере — оценку
        и подсказки по ходу игры, а после конца партии разбор всё равно восстанавливает
        оценку каждого хода задним числом.
      </p>
    ...
    <section class="article">
      ...
      <ol class="steps">
        ...
        <li>
          ...
          партию</a> — разбор посчитает ошибки и найдёт перелом даже в честной партии, где тренер
          во время игры не включался, и покажет, в какой момент удержание позиции подвело на самом деле.
        </li>
      </ol>
