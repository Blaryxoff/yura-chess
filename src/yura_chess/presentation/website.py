...
LANDING_BODY = f"""    <header>
      ...
      <p class="lead">
        ...
        с&nbsp;привычными шахматными фразами, понятными объяснениями и автоматическим сохранением партии.
        Экран не нужен.
      </p>
...
{LANDING_FAQ_HTML}
    ...
    <section>
      ...
      <p>
        ...
        запросов. Исходный идентификатор Алисы не сохраняется: вместо него используется необратимый код. Обезличенные
        счётчики использования сохраняются без срока для статистики, а текст распознанных команд — до 30 дней для
    ...
    <section class="article">
      ...
      <ol class="steps">
        <li>
          ...
          Скажите «Алиса, запусти навык Шахматы с Юрой». Навык бесплатный, устанавливать ничего не нужно.
        </li>
        <li>
      ...
      <p>
        ...
        Если Алиса неточно распознала фразу, скажите «что ты услышала» или «повтори координаты по буквам».
      </p>
      ...
      <p>
        ...
        Спросите «какой уровень», чтобы узнать текущий, и скажите «сыграем сложнее», чтобы поднять сложность
        в следующей партии.
      </p>

      <h3>Тренер, разбор и задачи</h3>
      <p>
        Скажите «включи режим тренера», чтобы получать оценку позиции, несколько подходящих ходов и подсказки
        от общего намёка к конкретному ходу.
...
COMMANDS_PAGE_HTML = _document(
    ...
    body=f"""    <header>
      ...
      <p class="lead">
        ...
        о чём можно попросить. Эти же разделы Алиса прочитает по команде «все команды».
      </p>
      <div class="hero-actions">
        <div class="launch">
...
ACCESSIBILITY_PAGE_HTML = _document(
    ...
    <section class="article">
      ...
      <p>
        Команда «какая позиция» читает доску по две горизонтали за раз, чтобы их было легче запомнить
        на слух; «дальше» продолжает чтение. Отдельные вопросы уточняют детали:
      ...
      <p>
        ...
        команды «повтори ответ» и «повтори медленно» повторяют предыдущий ответ, а «повтори координаты по буквам»
...
COACH_PAGE_HTML = _document(
    ...
    <section class="article">
      ...
      <p>
        ...
        После ответа «да» тренер сразу оценит позицию. Например:
      </p>
      <ul>
        <li>
          <strong>Вы:</strong> <code>«оцени позицию»</code> — партия ещё честная, тренер выключен.
          <strong>Навык:</strong> Сейчас играем без подсказок. Включить режим тренера и ответить
          на ваш вопрос? Это уже не честная партия. Скажите «да» или «нет».
        </li>
        <li>
          <strong>Вы:</strong> <code>«да»</code>.
          <strong>Навык:</strong> Включаю режим тренера. У вас небольшой перевес.
          Скажите «назови оценку числом», если нужна цифра.
        </li>
      </ul>
...
BLINDFOLD_PAGE_HTML = _document(
    ...
    <section class="article">
      ...
      <p>
        Попробуйте сыграть десять своих ходов без команды «какая позиция». Точечные вопросы
        о клетке, фигуре или последнем ходе разрешены.
      </p>
      <ol class="steps">
        <li>
          <strong>Цель</strong>
          Скажите «новая игра белыми уровень три» и сыграйте свои первые десять ходов,
          ни разу не спросив «какая позиция».
        </li>
        <li>
          <strong>Что можно</strong>
          Можно спрашивать «что на е четыре», «где белые слоны» и «какой был последний ход».
          Если понадобилось полное чтение доски, просто начните следующую попытку сначала.
        </li>
        <li>
          <strong>Разбор результата</strong>
          После попытки спросите «сколько я сделал ходов». Доиграйте партию до конца, затем попросите
      ...
      <ul>
        <li>Считайте, сколько ходов удаётся продержаться без команды «какая позиция»</li>
