    ...
    <section>
      ...
      <div class="grid">
        ...
        <div class="feature">
          ...
          Готовы обходиться без полного чтения доски — попробуйте
          <a href="{BLINDFOLD_PATH}">игру вслепую с измеримым упражнением</a>.
        </div>
        <div class="feature">
    ...
    <section class="article">
      ...
      <ol class="steps">
        <li>
          ...
          Скажите «Алиса, запусти навык Шахматы с Юрой» на любом устройстве, где Алиса поддерживает
          сторонние навыки. Навык бесплатный, устанавливать ничего не нужно.
        </li>
        <li>
      ...
      <p>
        Реальные фразы игроков звучат короче инструкции. Вот та же последовательность на практике,
        по расшифровкам голосовых запросов Алисы.
      </p>
      <ul>
        <li>
        ...
        <li>
          <strong>«Конев три»</strong>
          Распознавание иногда склеивает «конь эф» в одно слово и путает «ф» с «в» — навык всё равно
          понимает ход конём на эф три, если конь действительно может туда пойти.
        </li>
        <li>
      ...
      <p>
        <a href="{COACH_PATH}">Разбор реального цикла запрос — согласие — оценка тренера</a> показывает,
        что отвечает навык на «оцени позицию» до и после включения тренера. А
        <a href="{BLINDFOLD_PATH}">упражнение на память без полного чтения доски</a> даёт измеримую
        цель для тренировки без подглядывания в позицию.
      </p>
      ...
      <h3>На каких устройствах работает навык</h3>
      <p>
        Экран не обязателен ни для одного действия: навык не рисует ничего, чего нельзя было бы
        узнать голосом, доска на экране только дополняет ответы. Он запускается на любом устройстве
        Алисы, которое поддерживает сторонние навыки — независимо от того, есть на нём дисплей или нет.
      </p>
    </section>
""",
)
...
COACH_PAGE_HTML = _document(
    ...
    <section class="article">
      ...
      <p>
        Вопрос об оценке позиции посреди честной партии не получает оценку сразу — сначала навык
        спрашивает согласие включить тренера, и только после «да» отвечает по существу. Вот реальная
        последовательность запрос — согласие — фактический ответ для позиции с небольшим перевесом —
        одной из шести словесных оценок навыка, слова меняются вместе с позицией, а структура ответа нет:
      </p>
      <ul>
        <li>
      ...
      <p>
        Разбор доступен для любой завершённой партии — даже честной, где тренер во время игры
        ни разу не включался. В этом разница между тремя состояниями: в честной партии вопрос
        об оценке позиции получает только предложение включить тренера, при включённом тренере —
        оценку и подсказки по ходу игры, а после конца партии разбор всё равно восстанавливает
        оценку каждого вашего хода задним числом.
      </p>
...
BLINDFOLD_PAGE_HTML = _document(
    ...
    <section class="article">
      ...
      <p>
        Одна законченная попытка с понятной целью и понятным результатом — не общий совет,
        а конкретный сценарий на один присест.
      </p>
      <ol class="steps">
        <li>
          <strong>Цель</strong>
          «Новая игра белыми уровень три», затем сыграть свои первые десять ходов, ни разу
          не спросив «какая позиция» — это единственная страховка, которая запрещена в этом
          упражнении.
        </li>
        <li>
          <strong>Что можно</strong>
          Точечные уточнения не запрещены и не портят попытку: «что на е четыре», «где белые слоны»,
          «какой был последний ход». Под запретом только полное чтение доски по горизонталям —
          если оно понадобилось, попытка на этом заканчивается, а не откладывается.
        </li>
        <li>
          <strong>Разбор результата</strong>
          После десятого хода или срыва спросите «сколько ходов мы сыграли», чтобы зафиксировать
          факт. Доиграйте партию до конца, затем попросите тренера <a href="{COACH_PATH}">разобрать
          партию</a> — разбор посчитает шахматные ошибки даже в честной партии, где тренер во время
          игры не включался, и назовёт перелом, если он был. Момент, где именно расплылась
          картина в голове, разбор не видит — это игрок сопоставляет с переломом сам.
        </li>
      </ol>
